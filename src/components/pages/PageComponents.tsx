...
import {
  ...
  PageCTA,
  PageSection,
} from '@/components/pages/PageComponents';

export function MyPage() {
  return (
    <>
      <PageHero title="My Page" badge="Section" />
      <PageContent content={htmlContent} />
      <PageSection title="Features" centered>
        <PageFeatureList features={[...]} />
      </PageSection>
      <PageCTA title="Ready?" ctaLabel="Get Started" />
    </>
  );
}

// Example 2: Card grid layout
import { PageCard, PageGrid, PageLoadingState } from '@/components/pages/PageComponents';

export function ListPage() {
  const { data: items, isLoading } = useQuery(...);

  if (isLoading) return <PageLoadingState />;
  if (!items?.length) return <PageEmptyState title="No items" />;

  return (
    <PageSection title="Our Offerings">
      <PageGrid columns={3}>
        {items.map(item => (
          <PageCard
            key={item.id}
...
This library provides consistent, reusable components for all public pages.